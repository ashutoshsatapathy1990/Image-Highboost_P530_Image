...
int main()
{
	...
	Mat Image = imread("Expo.bmp", CV_LOAD_IMAGE_COLOR);
	...
	cl_mem ORI_clmem = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(uchar) * Image.rows * Image.cols * 3, NULL, &clStatus);
	...
	//Read Data from Host Buffer to Device Buffer Before Image Enhancement.
	clStatus = clEnqueueWriteBuffer(command_queue_highboost, ORI_clmem, CL_TRUE, 0, sizeof(uchar) * Image.rows * Image.cols * 3, ORI_Frame, 0, NULL, NULL);
